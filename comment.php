...
*/
   // $conn = new mysqli('localhost', 'root', '', 'ytCommentSystem');
    ...
    if (isset($_POST['addComment'])) {
        ...
        $connection->query("INSERT INTO posts (PostAuthor, Post_Content, PostDate) VALUES ('".$_SESSION['userID']."','$comment',NOW())");
        exit('success');
    }
/*
    if (isset($_POST['register'])) {
        ...
        if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
            ...
            if ($sql->num_rows > 0)
                ...
                $connection->query("INSERT INTO users (name,email,password,createdOn) VALUES ('$name', '$email', '$ePassword', NOW())");

                $sql = $connection->query("SELECT id FROM users ORDER BY id DESC LIMIT 1");
    ...
    if (isset($_POST['logIn'])) {
        ...
        if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
            $sql = $connection->query("SELECT id, password, name FROM users WHERE email='$email'");
            if ($sql->num_rows == 0)
                exit('failed');
            else {
                $data = $sql->fetch_assoc();
